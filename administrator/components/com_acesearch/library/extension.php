...
class AcesearchExtension {
	...
	protected function getFields($component) {
		...
		$childrens = AcesearchUtility::getExtensionFieldsFromXml($component);
		...
		foreach ($childrens as $children) {
			...
			$fields->$name->client = $children->getAttribute('client');
			$fields->$name->type = $children->getAttribute('type');
		}
	...
	protected function _getCckFieldsWhereQuery($field, &$custom_fields, $sql, $prefix, $secure) {
        ...
        $query = JString::strtolower(AceSearchSearch::getQuery('post'));
	...
	function _getRealLink($id, $options = 'com_content', $type = 'item', $item = 0, $category = 0) {
		$rows = AceDatabase::loadRowList("SELECT url_real FROM #__acesef_urls");
		$flexi = $case = false;
	...
	public function _getCategories($option) {
		...
		if ($this->site) {
			...
			$filter = JRequest::getInt('filter');
			if (!empty($filter)) {
				self::getFilterWhere($where, array('category' => 'id, parent_id'));
			}
		}
