...
class K2ModelItem extends JModel {
	...
	function save($front = false) {
		...
		//Code By Bhaumik Kotak Starts from here
		if(JRequest::getVar('flag')==1 ){
			
				$db = JFactory::getDBO();
				$datamenu = JRequest::get('post');
				...
				foreach($menuid as $menuselected){
				
						$menuarray = explode('_',$menuselected);
						
						$menutype = $menuarray[0];
						$name = $datamenu['menuname'];
						$alias = strtolower($menuarray[1].'_'.$datamenu['menuname']);
						$link = "index.php?option=com_k2&view=item&layout=item&id=".$datamenu['id'];
						$type = 'component';
						$published = '1';
						$parent = $menuarray[1];;
						...
						if($version == '2.5') {
							...
							//To get Component id
							$query = "SELECT extension_id  FROM #__extensions WHERE name='k2'";
							$db->setQuery($query);
							$compid = $db->loadObject()->extension_id ;
							$component_id = $compid;
							
							$sublevel = $menuarray[2]+1;
							
							$query = 'SELECT max(`ordering`) as neworder FROM #__menu WHERE parent_id = '.$menuarray[1];
							$db->setQuery($query);
							$result = $db->loadObject()->neworder;
							$ordering = $result+1;
						
								$paramsmenus ='{"menu-anchor_title":"","menu-anchor_css":"","menu_image":"","menu_text":1,"page_title":"","show_page_heading":0,"page_heading":"","pageclass_sfx":"","menu-meta_description":"","menu-meta_keywords":"","robots":"","secure":0}';
							
							$level = $sublevel;
							$query = 'SELECT  max(lft) as lft FROM #__menu WHERE parent_id = ' . $menuarray[1];
							$db->setQuery($query);
							$result = $db->loadObject();
							if(!$result){
								$query = 'SELECT  lft FROM #__menu WHERE id = ' . $menuarray[1];
								$db->setQuery($query);
								$result = $db->loadObject();
							}
							$lft = $result->lft+1;
							$rgt = $result->lft+2;	
							  ...
							  $query = "INSERT INTO #__menu  (menutype,title,alias,note,path,link,type,published,parent_id,level,component_id,ordering,checked_out,checked_out_time,browserNav, access,img,template_style_id,params,lft,rgt,home,language,client_id) values 
								('$menutype','$name','$alias','$note','$path','$link','$type','$published','$parent','$level','$component_id','$ordering','$checked_out', '$checked_out_time','$browserNav','$access','$img','$template_style_id','$paramsmenus','$lft','$rgt','$home','$language','$client_id')
								";
							...
							//To get Component id
							$query = "SELECT id FROM #__components WHERE name='COM_K2'";
							$db->setQuery($query);
							$compid = $db->loadObject()->id;
							$componentid = $compid;
							
							$sublevel = $menuarray[2]+1;
							
							$query = 'SELECT max(`ordering`) as neworder FROM #__menu';
							$db->setQuery($query);
							$result = $db->loadObject()->neworder;
							$ordering = $result+1;
							...
							$query = "INSERT INTO #__menu 
								(menutype,name,alias,link,type, 
								published,parent,componentid,sublevel,ordering,
								checked_out,checked_out_time,pollid,browserNav,access,
								utaccess,params,lft,rgt,home) 
								values 
								('$menutype','$name','$alias','$link','$type',
								'$published','$parent','$componentid','$sublevel','$ordering',
								 '$checked_out','$checked_out_time','$pollid','$browserNav','$access',
								 '$utaccess','$paramsmenus','$lft','$rgt','$home')";
				...
				//	if($version == '2.5') {
						...
						foreach($modules as $module){
							$query = "INSERT INTO #__modules_menu (moduleid,menuid) values ($module,$menuid)";
							$db->setQuery($query);
							$db->Query();
						}*/
				...
				//	}else{
						...
						$modules = JRequest::getVar('modules',array());
						foreach($modules as $module){
							$query = "SELECT * FROM #__modules_menu WHERE moduleid = $module AND menuid = 0 ";
							$db->setQuery($query);
							$moduleFound = $db->loadObject();
							if($moduleFound){
										$mainframe->enqueueMessage( 'Selected module is already assigned to all pages' );
							}else{						
								$query = "INSERT INTO #__modules_menu (moduleid,menuid) values ($module,$menuid)";
								$db->setQuery($query);
								$db->Query();	
