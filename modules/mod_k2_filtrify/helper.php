<?php
/**
 * @version		$Id: helper.php 1.3.12 2012-10-10 grigor.mihov $
 * @package		K2 Filtrify
...
if($params->get('loadjQuery') == "1"){
	$document->addScript(JURI::root(true).'/modules/mod_k2_filtrify/js/jquery-1.7.1.min.js');
;}
$document->addStyleSheet(JURI::root(true).'/modules/mod_k2_filtrify/css/style.css');
$document->addStyleSheet(JURI::root(true).'/modules/mod_k2_filtrify/css/animate.css');
$document->addStyleSheet(JURI::root(true).'/modules/mod_k2_filtrify/css/filtrify.css');
$document->addScript(JURI::root(true).'/modules/mod_k2_filtrify/js/filtrify.min.js');
//setting params
$k2FiltrifyCloseTag			= $params->get( 'closeAfterAddTag', '0' );
$k2NoResult					= $params->get( 'noresult', 'No results match:' );
$k2CallBackMethod			= $params->get( 'callBack', '\'\'' );
$paginationFunction 		= null;
$closingPaginationFunction	= null;
$paginationItemsPerPage		= $params->get( 'itemsPerPage', '5' );
$placeholder				= null;
$k2Hide						= null;
$closeAfterAddTag 			= null;
//setup close after adding tag, this remove bug with joomla 1.5
if($k2FiltrifyCloseTag == 0){
	$closeAfterAddTag = "false";
}
elseif($k2FiltrifyCloseTag == 1){
	$closeAfterAddTag = "true";
}
//set script if legend callback method is selected in the module params	
	// LEGEND Method
	if($k2CallBackMethod == 'legend'){
		$placeholder = $k2CallBackMethod;
		$k2CallBackMethod = 'function( query, match, mismatch ) {
									if ( !mismatch.length ) {
										jQuery("#legend").html("<i>Viewing all movies.</i>");
									} else {
										var category, tags, i, tag, legend = "<h4>Viewing:</h4>";
										for ( category in query ) {
											tags = query[category];
											if ( tags.length ) {
												legend += "<p class=\"selected\"><span class=\"category\">" + category + ": </span>";
												for ( i = 0; i < tags.length; i++ ) {
													tag = tags[i];
													legend += "<em class=\"element\">" + tag + "</em>";
												};
												legend += "</p>";
											};
										};
										legend += "<p class=\"result\"><i>" + match.length + " found.</i></p>";
	...
	// HIGHLIGHT Method
	elseif($k2CallBackMethod == 'highlighting'){ 
		$k2Hide = "hide		:	false,";
		$placeholder = $k2CallBackMethod;
		$k2CallBackMethod = ' function( query, match, mismatch ) {
									if ( mismatch.length ) {
											jQuery( mismatch ).removeClass("highlight").addClass("fade");
											jQuery( match ).removeClass("fade").addClass("highlight");
										} else {
											jQuery( match ).removeClass("fade highlight");
								};
       						 }
						';
	}
	...
	// PAGINATION Method
	elseif($k2CallBackMethod == 'pagination'){ 
		$document->addScript(JURI::root(true).'/modules/mod_k2_filtrify/js/jPages.min.js');
		$document->addScript(JURI::root(true).'/modules/mod_k2_filtrify/js/jquery.lazyload.min.js');
		$placeholder = $k2CallBackMethod;
		$k2CallBackMethod = ' function() {
									destroyPagination();
									setPagination();
								}
							';
		$paginationFunction ='jQuery(function() {
									var container = jQuery("#k2FiltrifyContainer"),
									...
									function setPagination () {
										pagination.jPages({
											containerID : "k2FiltrifyContainer",
									...
									setPagination();
								';
			$closingPaginationFunction = '});';					
	};
	
//adding params to the jQuery plugin
	...
	"<script type=\"text/javascript\">
		jQuery.noConflict()(function(){
			jQuery(document).ready(function(){
				...
				jQuery.filtrify(\"k2FiltrifyContainer\", \"k2FiltrifyPlaceHolder\",{
					".$k2Hide."
					block 		: 	\"data-original\",
					close 		: 	".$k2FiltrifyCloseTag.",
					noresults 	: 	\"".$k2NoResult."\",
					callback 	:	".$k2CallBackMethod."
...
//End of Filtrify scripts

class modK2FiltrifyHelper
{
