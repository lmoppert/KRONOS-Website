		...
		this._items.each( this._bind( function( index, element ) {
			...
			for ( i = 0 ; i < attr.length; i++ ) {
				...
				if ( name.indexOf( "data-" ) === 0 && $.inArray( name, this.options.block ) === -1 ) {
					field = name.replace(/data-/gi, "").replace(/-/gi, " ").split('_').join(' '); //me7hos adding .split(' ').join('_') to have fields with spaces
...
$(window).load(function(){
	 var max = -1;
$("#k2FiltrifyContainer li.itemContainer").each(function() {
    var h = $(this).height(); 
    max = h > max ? h : max;
});
$('#k2FiltrifyContainer li.itemContainer').css('min-height', max);
});
