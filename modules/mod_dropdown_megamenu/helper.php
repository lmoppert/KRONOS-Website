...
*/
			...
			if ($tmp->type == 'separator')
			{
				...
				if ($tmp->url != null)
				{
					switch ($tmp->browserNav)
					{
						...
						case 0:
							// _top
							$data = '<a href="'.$tmp->url.'" '.$active.' '.$id.' '.$title.'>'.$txt.'</a></li>';
							break;
						case 1:
							// _blank
							$data = '<a href="'.$tmp->url.'" target="_blank" '.$active.' '.$id.' '.$title.'>'.$txt.'</a></li>';
							break;
						case 2:
							// window.open
							...
							$data = '<a href="'.$link.'" onclick="window.open(this.href,\'targetWindow\',\''.$attribs.'\');return false;" '.$active.' '.$id.' '.$title.'>'.$txt.'</a></li>';
							break;
					}
				} else {
					$data = '<a '.$active.' '.$id.' '.$title.'>'.$txt.'</a></li>';
				}
			}
		}
	...
	function beginMenuItem($mitem=null, $level = 0, $pos = '', $itemid){
		...
		if ($active) $active = " class=\"$active\"";
		echo "<li id=\"clickid".$itemid->id."\" onclick=\"showHideMenu(".$itemid->id.");\" $active>";
	...
	function endMenuItems1($pid=0, $level=0, $return=false){
		$data = "<div class=\"menuclose\"><a href=\"#\" onclick=\"closeMenu();\"><img src=\"/modules/mod_dropdown_megamenu/assets/images/close.png\" onmouseover=\"this.src='/modules/mod_dropdown_megamenu/assets/images/close_hover.png'\" onmouseout=\"this.src='/modules/mod_dropdown_megamenu/assets/images/close.png'\" width=\"24px;\" /></a></div>";
